 ...
 * @author ianfr
 */
public class SaldosProduccionFlotaSolymarController {
    ...
    public SaldosProduccionFlotaSolymarController(FlotaView view, ReporterVBView frame) {
        this.view = view;
        this.frame = frame;
    ...
    private void jasper(boolean flag) {
        ...
        try {
            ...
            jasperReport = JasperCompileManager.compileReport("jrxml/INF-LiquidacionSaldosxFlotaSolyMar.jrxml");
            Conexion con = new Conexion();

            JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, params, con.getConnection());

            if (flag) {
                this.frame.setNewReport(new JRViewer(jasperPrint), "Saldos x Flota Sol y Mar");
    ...
    private void jasperIndividual(boolean flag) {
        ...
        try {
            ...
            jasperReport = JasperCompileManager.compileReport("jrxml/INF-LiquidacionSaldosxFlotaIndividualSolyMar.jrxml");
            Conexion con = new Conexion();

            JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, params, con.getConnection());

            if (flag) {
                this.frame.setNewReport(new JRViewer(jasperPrint), "Saldos x Flota:" + flota.getNombreFlota());
                System.err.println("VIEW");
            } else {
                JasperPrintManager.printReport(jasperPrint, true);
                System.err.println("PRINT");
